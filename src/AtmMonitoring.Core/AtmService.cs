...
/// <summary>
/// AtmService provides operations to manage ATM states.
...
public sealed class AtmService : IAtmService
{
    ...
    /// <summary>
    /// Returns all ATMs. Optimized to avoid accessing ConcurrentDictionary.Values directly.
    /// Accessing .Values creates a snapshot of the dictionary, allocating an internal array/list
    /// of size O(N). By iterating the dictionary directly and yielding elements, we avoid
    /// this snapshot allocation, achieving O(1) allocation overhead (just the iterator state machine).
    /// </summary>
