using System.Collections.Concurrent;
using BenchmarkDotNet.Attributes;
using AtmMonitoring.Core;

namespace AtmMonitoring.Benchmarks;

[MemoryDiagnoser]
public class DictionaryBenchmarks
{
    private readonly ConcurrentDictionary<string, Atm> _atms = new();

    [Params(10, 100, 1000)]
    public int ItemCount { get; set; }

    [GlobalSetup]
    public void Setup()
    {
        _atms.Clear();
        for (int i = 0; i < ItemCount; i++)
        {
            var id = $"ATM{i:D3}";
            _atms.TryAdd(id, new Atm
            {
                Id = id,
                Location = $"Location {i}",
                Status = AtmStatus.Online
            });
        }
    }

    [Benchmark(Baseline = true)]
    public List<Atm> ConcurrentDictionaryValuesSnapshot()
    {
        // This is what _atms.Values does under the hood or what is returned when accessed directly
        var list = new List<Atm>();
        foreach (var value in _atms.Values)
        {
            list.Add(value);
        }
        return list;
    }

    [Benchmark]
    public List<Atm> DirectIterationYieldReturn()
    {
        var list = new List<Atm>();
        foreach (var atm in IterateDirectly())
        {
            list.Add(atm);
        }
        return list;
    }

    private IEnumerable<Atm> IterateDirectly()
    {
        foreach (var kvp in _atms)
        {
            yield return kvp.Value;
        }
    }
}
